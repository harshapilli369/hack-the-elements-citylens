...
const HISTORY_SAMPLE_EVERY = 60  // record one data point per second at 60fps
...
export const useCityFlowStore = create((set, get) => ({
  ...
  triggerDisaster: (cityId, type) => {
    set(state => {
      ...
      const newCities = state.cities.map(c => {
        if (c.id === cityId) {
          return { ...c, isDisasterActive: true, disasterType: type, disasterTimer: 1000 }
        }
        return c
      })
  ...
  tick: () => {
    ...
    set(prev => {
      ...
      // Cascade Effect & stress calculation
      updatedCities = updatedCities.map(city => {
        let updatedCity = { ...city }
        updatedCity.stress = (
          updatedCity.resources.housing * 0.20 +
          updatedCity.resources.water   * 0.20 +
        ...
        // Sample history once per second (every HISTORY_SAMPLE_EVERY ticks)
        if (newTickCount % HISTORY_SAMPLE_EVERY === 0) {
          const ecoHealth = Math.max(0, 100 - updatedCity.stress)
          const newPoint = {
            pop:    Math.round(updatedCity.pop / 1000 * 10) / 10,
            stress: Math.round(updatedCity.stress * 10) / 10,
            eco:    Math.round(ecoHealth * 10) / 10,
          }
          const newHistory = [...(updatedCity.history || []), newPoint]
          updatedCity.history = newHistory.length > HISTORY_MAX
