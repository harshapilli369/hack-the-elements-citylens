...
export default function CityFlowSimulator() {
  ...
  return (
    <div className="cityflow-container w-full h-screen overflow-hidden flex flex-col">
      {/* Top Navigation Bar */}
      <header className="h-16 flex-none flex items-center justify-between px-6 border-b border-slate-800 bg-[#0b1120]/80 backdrop-blur-md z-30">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-gradient-to-br from-sky-400 to-blue-600 flex items-center justify-center font-bold text-lg shadow-[0_0_15px_rgba(56,189,248,0.4)]">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
            </svg>
          </div>
          <div>
            <h1 className="text-lg font-bold leading-tight">CityFlow <span className="text-sky-400">Simulator</span></h1>
            <div className="text-[10px] uppercase tracking-[0.2em] text-slate-400">Climate Migration & Infrastructure Cascade</div>
          </div>
        </div>
        ...
        <div className="flex gap-6">
          <div className="cityflow-glass px-4 py-1.5 rounded-full flex gap-3 border-slate-700">
            <div className="flex flex-col items-center">
              ...
              <span className="font-mono font-bold leading-none">{citiesCount}</span>
            </div>
            <div className="w-px bg-slate-700" />
            <div className="flex flex-col items-center">
              <span className="text-[9px] uppercase tracking-wider text-slate-400">In Transit</span>
              <span className="font-mono font-bold leading-none">{(totalInTransit / 1000).toFixed(1)}k</span>
            </div>
            <div className="w-px bg-slate-700" />
            <div className="flex flex-col items-center">
              <span className="text-[9px] uppercase tracking-wider text-slate-400">Active Disasters</span>
              <span className="font-mono font-bold leading-none text-red-400">{activeDisasters}</span>
            </div>
          </div>
        </div>
      </header>
